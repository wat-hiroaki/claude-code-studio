...
>
> **Mission**: Claude Code の"見えない"を"見える"にする管制塔。
> Claude Code が実行エンジンなら、CCS は管制塔 — 飛行機を操縦するのではなく、飛行状態を観測し全体像を把握する。
...
### Phase 4: Stability & UX Polish

UI の安定性と基本的な使い勝手を改善する。外部ユーザーが増え始めた段階であり、
「動くけど挙動が怪しい」を「安心して使える」に引き上げるフェーズ。

21. [ ] **Collapsible sidebar** — 左サイドバーの折りたたみ/展開トグル（#2）
22. [ ] **Panel state persistence** — SSHパネル等の位置移動が正しく保存・復元される（#3）
23. [ ] **Navigation rendering fix** — Dashboard→戻る時の画面ブランク問題の修正（#4）
24. [ ] **Claude Code feature sync** — 新機能（Remote agents, Triggers, Worktrees, Agent SDK 等）を設定ファイルから動的検出し、ConfigMap に自動反映
25. [ ] **MCP Resources browser** — MCP Tools だけでなく Resources（データソース）も閲覧可能に
26. [ ] **Settings file watcher** — `settings.json`, `.claude/` の変更をファイル監視で即時反映（手動リロード不要）

### Phase 5: Observability（管制塔としての進化）

CCS の核心価値「Claude Code の"見えない"を"見える"にする」を深化させるフェーズ。
Anthropic のハーネスエンジニアリングの知見を取り込み、マルチエージェント運用時の
可視化を強化する。ただし CCS はあくまで**観測ツール**であり、ハーネスの設計・構築は
Claude Code 自体の責務。

```
[設計思想のレイヤー]

L1: Anthropic Claude Code
  → コンテキスト設計、ツール権限、MCP
  → CCSの対応: ConfigMap, Agent Profile View

L2: Agentic Org（McKinsey）
  → 組織メタファー、CEO→部門→チーム階層
  → CCSの対応: Dashboard 組織図, Workspace 分離

L3: Harness Engineering（Anthropic）
  → Planner/Generator/Evaluator 分離、構造化ハンドオフ、基準ベース評価
  → CCSの対応: Phase 5 で可視化（設計UIは作らない）

[CCS の立ち位置]
  Claude Code が「実行エンジン」なら、CCS は「管制塔」。
  管制塔は飛行機を操縦しない。飛行状態を観測し、全体像を把握する。
```

27. [ ] **Agent phase indicator** — エージェントの現在フェーズ表示（Planning / Generating / Evaluating / Idle）をステータス検出パターンの拡張で実現
28. [ ] **Handoff timeline** — Task Chain 実行時のエージェント間ハンドオフ履歴をタイムライン表示
29. [ ] **Session analytics** — セッション毎のトークン使用量・実行時間・ツール呼び出し回数の集計
30. [ ] **Knowledge Base panel** — `.claude/product-marketing-context.md` 等のドキュメントをGUIで閲覧・編集（開発以外の業務コンテキストも管理可能に）
31. [ ] **Scheduler history** — Remote Triggers / Cron 実行の履歴・結果表示
32. [ ] **Hook fire log** — Hooks の発火履歴とトリガー条件の可視化

### Phase 6: Ecosystem（将来ビジョン）

Claude Code エコシステムの進化に追従しつつ、CCS 独自の価値を拡張する長期ビジョン。
実装時期は Claude Code 側の機能成熟度に依存。

33. [ ] **Worktree manager** — 並列開発環境（git worktrees）の一覧・切替UI
34. [ ] **Agent hierarchy view** — Orchestrator → subagent の指揮命令系統を組織図で可視化
35. [ ] **Cross-workspace overview** — 全ワークスペースの健全性を一画面で俯瞰
36. [ ] **Plugin system** — サードパーティによる CCS 拡張（カスタムパネル、データソース）
37. [ ] **Mobile companion** — 出先からエージェント状態を確認できる軽量Web UI
