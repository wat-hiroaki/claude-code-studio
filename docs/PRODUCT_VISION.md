# Claude Code Desktop — Product Vision
...
3. ✅ **tmux integration** — セッション作成・アタッチ・デタッチ、自動命名 (ccd-\<ws\>-\<agent\>)
...
[tmux セッション命名規則]
ccd-<workspace>-<agent>
例: ccd-companyA-cpo, ccd-personal-dev
```
...
[リモート保存]
~/.claude-code-desktop/ on remote host
└── (同じ構造)
...
```bash
# ccd-bootstrap.sh — アプリから SSH 経由で実行
#!/bin/bash
set -e

echo "=== Claude Code Desktop — Remote Setup ==="
...
tmux は複数クライアントの同時アタッチをサポート:
  ...
  PC-A: ssh remote → tmux attach -t ccd-companyA-cpo
  PC-B: ssh remote → tmux attach -t ccd-companyA-cpo
    → 両方に同じ画面がリアルタイム表示
    → どちらからでも入力可能
