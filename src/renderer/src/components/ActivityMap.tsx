...
// ---------------------------------------------------------
// CYBER/HUD THEME DEFINITIONS (extends shared palette with ActivityMap-specific props)
// ---------------------------------------------------------
            ...
            {activeAgents.map((agent) => {
              ...
              return (
                <g key={agent.id}>
                  ...
                  <AgentNode agent={agent} x={pos.x} y={pos.y} onClick={handleAgentNodeClick} palette={palette} statusTheme={statusTheme} workspaceName={resolveWorkspaceName(agent)} memoryMB={agentMemory[agent.id] || 0} />
                </g>
              )
            })}
          ...
          >
            ...
            <div className="shrink-0 border-b px-3 py-2 grid grid-cols-2 gap-x-4 gap-y-1" style={{ borderColor: palette.cockpitBorder, backgroundColor: palette.cockpitHeaderBg }}>
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-mono text-[10px] font-medium" style={{ color: (agentMemory[cockpitAgent.id] || 0) > 2048 ? palette.red : (agentMemory[cockpitAgent.id] || 0) > 1024 ? palette.orange : palette.textMain }}>
                  {(() => { const mb = agentMemory[cockpitAgent.id] || 0; return mb >= 1024 ? `${(mb / 1024).toFixed(1)}GB` : `${mb}MB` })()}
                </span>
              </div>
              <div className="flex items-center gap-1.5">
